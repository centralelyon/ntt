...
def cut_video(
    ...
    Args:
        video_file_in (Path or str): Path to the input video file
        video_file_out (Path or str): Path to the output video file
    ...
    Returns:
        str: _description_
    """
    myclip_in = VideoFileClip(str(video_file_in))
    myclip_out = myclip_in.subclip(start, end)
    myclip_out.write_videofile(str(video_file_out))
    myclip_in.close()
    myclip_out.close()
